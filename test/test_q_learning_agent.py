...
def test_q_learning_agent_exploitation():
    ...
    agent.q_table[state_key] = {
        "AHEAD": 0.2, "TURN_LEFT": 1.0, "TURN_RIGHT": 0.5
    }
    selected_action = agent.select_action(state_key, REL)
...
def test_q_learning_agent_learning():
    ...
    agent.q_table[next_state_key] = {
        "AHEAD": 2.0, "TURN_LEFT": 1.0, "TURN_RIGHT": 0.5
    }
    agent.learn(
        state_key, "TURN_LEFT", reward=1.0,
        next_state_key=next_state_key, done=False
    )
...
def test_q_learning_agent_epsilon_decay():
    agent = QLearningAgent(
        q_table={}, epsilon=1.0, epsilon_decay=0.5, epsilon_min=0.1
    )
    agent.decay_epsilon()
...
def test_q_learning_agent_epsilon_decay_disabled():
    agent = QLearningAgent(
        q_table={}, learning_enabled=False,
        epsilon=1.0, epsilon_decay=0.5, epsilon_min=0.1
    )
    agent.decay_epsilon()
